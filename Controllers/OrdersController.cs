...
public class OrdersController : ControllerBase
{
        ...
        CreateOrderDto dto)
    {
        ...
        // ─── Notification Service ला call करूया ────────────────────
        try
        {
            ...
            var response = await _httpClient.PostAsJsonAsync(
                "api/notification/order-placed", notification);

            if (response.IsSuccessStatusCode)
                _logger.LogInformation(
                    "✅ Notification sent for Order: {OrderId}", order.Id);
            else
                _logger.LogWarning(
                    "⚠️ Notification failed for Order: {OrderId}", order.Id);
        }
        catch (Exception ex)
        {
            // Notification fail झाली तरी Order cancel होणार नाही!
            _logger.LogWarning(
                "⚠️ Notification service unavailable: {Message}", ex.Message);
        }
